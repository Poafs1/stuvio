...
def denselayer(inputs, output_size):
    ...
    denseLayer = tf.compat.v1.layers.Dense(output_size, activation=None, kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
...
def upscale_four(inputs, scope='upscale_four'): # mimic the tensorflow bilinear-upscaling for a fix ratio of 4
    with tf.compat.v1.variable_scope(scope):
        ...
        hi_res_reshape = tf.transpose( a=hi_res_reshape, perm=(0,1,3,2,4,5) )
...
def random_flip(input, decision):
    f1 = tf.identity(input)
    f2 = tf.image.flip_left_right(input)
    output = tf.cond(pred=tf.less(decision, 0.5), true_fn=lambda: f2, false_fn=lambda: f1)
...
def tf_data_gaussDownby4( HRdata, sigma = 1.5 ):
    ...
    with tf.device('/gpu:0'):
        ...
        cur_data = tf.nn.conv2d(input=HRdata, filters=fix_gkern, strides=[1,4,4,1], padding="VALID", name='gauss_downsample_4')
